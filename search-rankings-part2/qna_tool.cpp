...
void QNA_tool::query(string question, string filename){
    ...
    //Note first word is ignored as P(first word is imp word) < E
    vector<int> importance;
    for(int i = 1 ; i<words.size();i++){
        // int in_mkg = mkg.get_word_count(words[i]);
        ...
        if(words[i].substr(1,6)=="ahatma" || words[i].substr(1,4)=="ndia" || words[i].substr(1,5)=="andhi" || max > ignorable) continue;
        if(idx>=26 && idx<36){
